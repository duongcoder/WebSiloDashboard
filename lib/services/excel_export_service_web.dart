...
Future<ExcelExportResult> exportAlertsToExcel({
  ...
  try {
    ...
    final fileName = (downloadFileName != null && downloadFileName.trim().isNotEmpty)
        ? downloadFileName.trim()
        : 'CanhBao_$day-$month-${year}_$hour-$minute-$second.xlsx';

    final bytes = _buildAlertsExcel(items: items);
    if (bytes == null) {
      return const ExcelExportResult(
        success: false,
        message: 'Không thể tạo dữ liệu Excel.',
    ...
    return _downloadByAnchor(bytes: Uint8List.fromList(bytes), fileName: fileName);
  } catch (e) {
    return ExcelExportResult(
      success: false,
      message: 'Xuất Excel thất bại: $e',
